#!/usr/bin/env node
import fs from "node:fs";
import path from "node:path";
import readline from "node:readline";
import { execSync } from "node:child_process";

function makeRl() {
  return readline.createInterface({ input: process.stdin, output: process.stdout });
}
function ask(rl, q) {
  return new Promise((resolve) => rl.question(q, (ans) => resolve(ans.trim())));
}

function pad2(n) {
  return String(n).padStart(2, "0");
}
function formatDateYmd(d) {
  return `${d.getFullYear()}-${pad2(d.getMonth() + 1)}-${pad2(d.getDate())}`;
}
function formatDateYmdHms(d) {
  return `${formatDateYmd(d)}_${pad2(d.getHours())}${pad2(d.getMinutes())}${pad2(
    d.getSeconds()
  )}`;
}

// ローカルTZ(+09:00等)を含む ISO 文字列
function formatIsoWithOffset(d) {
  const yyyy = d.getFullYear();
  const mm = pad2(d.getMonth() + 1);
  const dd = pad2(d.getDate());
  const HH = pad2(d.getHours());
  const MM = pad2(d.getMinutes());
  const SS = pad2(d.getSeconds());

  const offsetMin = -d.getTimezoneOffset();
  const sign = offsetMin >= 0 ? "+" : "-";
  const abs = Math.abs(offsetMin);
  const oh = pad2(Math.floor(abs / 60));
  const om = pad2(abs % 60);

  return `${yyyy}-${mm}-${dd}T${HH}:${MM}:${SS}${sign}${oh}:${om}`;
}

// 例: "未来 テスト" -> "future-test"（日本語も残す）
function slugify(input) {
  return input
    .toLowerCase()
    .trim()
    .replace(/[^\p{L}\p{N}\s-]/gu, "") // 記号を削除（日本語も残す）
    .replace(/\s+/g, "-")
    .replace(/-+/g, "-")
    .replace(/^-|-$/g, "");
}

function ensureDir(dir) {
  fs.mkdirSync(dir, { recursive: true });
}
function fileExists(p) {
  try {
    fs.accessSync(p, fs.constants.F_OK);
    return true;
  } catch {
    return false;
  }
}

function parsePublishAtRaw(publishAtRaw) {
  // "YYYY-MM-DD HH:mm" or "YYYY-MM-DD" をローカルとして解釈
  const m = publishAtRaw.match(
    /^(\d{4})-(\d{2})-(\d{2})(?:\s+(\d{2}):(\d{2}))?$/
  );
  if (!m) return null;
  const yyyy = Number(m[1]);
  const mm = Number(m[2]) - 1;
  const dd = Number(m[3]);
  const HH = m[4] ? Number(m[4]) : 0;
  const MM = m[5] ? Number(m[5]) : 0;
  return new Date(yyyy, mm, dd, HH, MM, 0);
}

function uniqueFilePath(dir, baseName) {
  const ext = path.extname(baseName);
  const stem = baseName.slice(0, -ext.length);

  let p = path.join(dir, baseName);
  if (!fileExists(p)) return p;

  for (let i = 1; i <= 99; i++) {
    const suffix = `_${String(i).padStart(2, "0")}`;
    const candidate = path.join(dir, `${stem}${suffix}${ext}`);
    if (!fileExists(candidate)) return candidate;
  }
  const now = new Date();
  return path.join(dir, `${stem}_${formatDateYmdHms(now)}${ext}`);
}

function safeExec(cmd) {
  execSync(cmd, { stdio: "inherit" });
}
function safeExecTry(cmd) {
  try {
    execSync(cmd, { stdio: "inherit" });
    return true;
  } catch {
    return false;
  }
}

const ALLOWED_SOURCES = ["internal", "youtube", "twitch", "vrchat", "kakuyomu", "note", "x"];
const ALLOWED_VIS = ["public", "unlisted", "draft"];

/**
 * CLI
 *   npm run new:activity -- "タイトル"
 *
 * Options:
 *   --desc "説明"
 *   --slug "custom-slug"
 *   --publish "YYYY-MM-DD HH:mm"     (空なら今)
 *   --visibility public|unlisted|draft
 *   --source internal|youtube|twitch|vrchat|kakuyomu|note|x
 *
 * 推奨（将来が楽）:
 *   --url "https://..."              external.url
 *   --platform "youtube"            external.platform（省略なら source を使う）
 *   --activity "devlog,stream,..."  tagsV1.activity（カンマ区切り）
 *   --world "Observers,Shi-chan"    tagsV1.world
 *   --system "astro,github"         tagsV1.system
 *   --theme "post-work"             tagsV1.theme
 *
 * Other:
 *   --open
 *   --git
 *   --push
 */
function parseArgs(argv) {
  const args = argv.slice(2);
  const out = {
    title: "",
    desc: "",
    slug: "",
    publish: "",
    visibility: "public",
    source: "internal",

    url: "",
    platform: "",

    // tagsV1
    tActivity: "",
    tWorld: "",
    tSystem: "",
    tTheme: "",

    open: false,
    git: false,
    push: false,
    help: false,
  };

  const takeValue = (i) => (i + 1 < args.length ? args[i + 1] : "");
  for (let i = 0; i < args.length; i++) {
    const a = args[i];
    if (a === "--help" || a === "-h") out.help = true;
    else if (a === "--desc") {
      out.desc = takeValue(i);
      i++;
    } else if (a === "--slug") {
      out.slug = takeValue(i);
      i++;
    } else if (a === "--publish") {
      out.publish = takeValue(i);
      i++;
    } else if (a === "--visibility") {
      out.visibility = takeValue(i);
      i++;
    } else if (a === "--source") {
      out.source = takeValue(i);
      i++;
    } else if (a === "--url") {
      out.url = takeValue(i);
      i++;
    } else if (a === "--platform") {
      out.platform = takeValue(i);
      i++;
    } else if (a === "--activity") {
      out.tActivity = takeValue(i);
      i++;
    } else if (a === "--world") {
      out.tWorld = takeValue(i);
      i++;
    } else if (a === "--system") {
      out.tSystem = takeValue(i);
      i++;
    } else if (a === "--theme") {
      out.tTheme = takeValue(i);
      i++;
    } else if (a === "--open") out.open = true;
    else if (a === "--git") out.git = true;
    else if (a === "--push") out.push = true;
    else if (!a.startsWith("--") && !out.title) out.title = a;
  }

  if (out.push) out.git = true;

  if (!ALLOWED_VIS.includes(out.visibility)) out.visibility = "public";
  if (!ALLOWED_SOURCES.includes(out.source)) out.source = "internal";

  // platform は省略なら source を使う（外部系で便利）
  if (!out.platform) out.platform = out.source;

  return out;
}

function usage() {
  return `
Usage:
  npm run new:activity -- "Title"
  npm run new:activity -- "Title" --source youtube --url "https://..." --activity "stream,archive"
  npm run new:activity -- "Title" --source internal --activity "devlog,ops" --system "astro,actions"

Options:
  --desc "..."                   Description（任意）
  --slug "..."                   slug を指定（任意）
  --publish "YYYY-MM-DD HH:mm"   publishAt（任意。省略で今）
  --visibility public|unlisted|draft
  --source internal|youtube|twitch|vrchat|kakuyomu|note|x

Recommended (future-proof):
  --url "https://..."            external.url
  --platform "youtube"           external.platform（省略なら source）
  --activity "a,b"               tagsV1.activity（カンマ区切り）
  --world "a,b"                  tagsV1.world
  --system "a,b"                 tagsV1.system
  --theme "a,b"                  tagsV1.theme

Other:
  --git                          git add/commit まで実行
  --push                         （--git含む）git pull --rebase & git push も実行
  --open
  --help
`.trim();
}

function csvToArray(s) {
  if (!s) return [];
  return s
    .split(",")
    .map((x) => x.trim())
    .filter(Boolean);
}

async function interactiveFallback(current) {
  const rl = makeRl();

  const onSigint = () => {
    console.log("\n⛔ Cancelled.");
    try {
      rl.close();
    } catch {}
    process.exit(130);
  };
  process.once("SIGINT", onSigint);

  try {
    const title = current.title || (await ask(rl, "Title（必須）: "));
    if (!title) return { ...current, title: "" };

    const desc = current.desc || (await ask(rl, "Description（任意・空でOK）: "));

    const source =
      current.source ||
      (await ask(
        rl,
        `source（任意）: [${ALLOWED_SOURCES.join("|")}] / 空=internal: `
      ));
    const src = ALLOWED_SOURCES.includes(source) ? source : "internal";

    const url =
      current.url ||
      (await ask(rl, 'external.url（任意）: 例 "https://...": '));
    const platform =
      current.platform ||
      (await ask(rl, `external.platform（任意）: 空=${src}: `));

    const publish =
      current.publish ||
      (await ask(rl, 'publishAt（任意）: 空=今 / 例 "2026-02-01 21:00": '));

    const slugSeed =
      current.slug || (await ask(rl, "slug（任意）: 空=タイトルから自動生成: "));

    const visibility = current.visibility || "public";

    const tActivity =
      current.tActivity || (await ask(rl, 'tagsV1.activity（任意）: 例 "devlog,ops": '));
    const tWorld =
      current.tWorld || (await ask(rl, 'tagsV1.world（任意）: 例 "Shi-chan": '));
    const tSystem =
      current.tSystem || (await ask(rl, 'tagsV1.system（任意）: 例 "astro,github": '));
    const tTheme =
      current.tTheme || (await ask(rl, 'tagsV1.theme（任意）: 例 "post-work": '));

    return {
      ...current,
      title,
      desc,
      source: src,
      url,
      platform: platform || src,
      publish,
      slug: slugSeed,
      visibility,
      tActivity,
      tWorld,
      tSystem,
      tTheme,
    };
  } finally {
    process.removeListener("SIGINT", onSigint);
    rl.close();
  }
}

async function main() {
  const opts0 = parseArgs(process.argv);
  if (opts0.help) {
    console.log(usage());
    return;
  }

  const needInteractive = !opts0.title;
  const opts = needInteractive ? await interactiveFallback(opts0) : opts0;

  if (!opts.title) {
    console.error("❌ Title は必須です。");
    process.exitCode = 1;
    return;
  }

  const contentDir = path.join(process.cwd(), "src", "content", "activities");
  ensureDir(contentDir);

  const now = new Date();

  // publishAt
  let publishAtDate = now;
  if (opts.publish) {
    const d = parsePublishAtRaw(opts.publish);
    if (!d) {
      console.error('❌ publishAt形式が不正です。例: "2026-02-01 21:00" または空');
      process.exitCode = 1;
      return;
    }
    publishAtDate = d;
  }

  // slug
  const slug = slugify(opts.slug || opts.title);
  if (!slug) {
    console.error("❌ slug を生成できませんでした（タイトルが特殊すぎる可能性）");
    process.exitCode = 1;
    return;
  }

  // filename: ymd_slug.md（衝突したら _01,_02...）
  const ymd = formatDateYmd(now);
  const baseName = `${ymd}_${slug}.md`;
  const filepath = uniqueFilePath(contentDir, baseName);

  const publishAtIso = formatIsoWithOffset(publishAtDate);
  const createdAtIso = formatIsoWithOffset(now);

  const tagActivity = csvToArray(opts.tActivity);
  const tagWorld = csvToArray(opts.tWorld);
  const tagSystem = csvToArray(opts.tSystem);
  const tagTheme = csvToArray(opts.tTheme);

  // external を “必要時だけ出す”
  const hasExternal = Boolean(opts.url);
  const externalBlock = hasExternal
    ? `
external:
  platform: "${opts.platform}"
  url: "${opts.url}"`
    : "";

  const body = `---
type: "activity"
source: "${opts.source}"
title: ${JSON.stringify(opts.title)}
description: ${opts.desc ? JSON.stringify(opts.desc) : '""'}
publishAt: "${publishAtIso}"
createdAt: "${createdAtIso}"
visibility: "${opts.visibility}"

tagsV1:
  world: ${JSON.stringify(tagWorld)}
  system: ${JSON.stringify(tagSystem)}
  activity: ${JSON.stringify(tagActivity)}
  theme: ${JSON.stringify(tagTheme)}

relations: []${externalBlock}
---

# ${opts.title}

（ここに本文）
`;

  fs.writeFileSync(filepath, body, "utf8");

  console.log("✅ Created:");
  console.log(`- ${filepath}`);

  if (opts.git) {
    const rel = path.relative(process.cwd(), filepath).replaceAll("\\", "/");
    console.log("");
    console.log("🔧 git automation:");

    safeExec(`git add -- "${rel}"`);

    const msg = `feat: add activity ${slug}`;
    safeExec(`git commit -m "${msg}"`);

    if (opts.push) {
      console.log("");
      console.log("🔁 sync before push: git pull --rebase");
      const ok = safeExecTry(`git pull --rebase`);

      if (!ok) {
        console.log("");
        console.log("⚠️ rebase failed. You may have conflicts.");
        console.log("Resolve conflicts, then run:");
        console.log("  git add .");
        console.log("  git rebase --continue");
        console.log("  git push");
        process.exitCode = 1;
        return;
      }

      console.log("");
      console.log("🚀 pushing...");
      safeExec(`git push`);
    }

    console.log("✅ git done.");
  } else {
    console.log("");
    console.log("Next:");
    console.log("1) 内容を書く");
    console.log(
      '2) git add . && git commit -m "feat: add activity" && git pull --rebase && git push'
    );
  }

  if (opts.open) {
    console.log("");
    console.log("Open (path):");
    console.log(filepath);
  }
}

main();
